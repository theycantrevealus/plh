...
class DSR extends Utility
{
  ...
  public function __POST__($parameter = array())
  {
    switch ($parameter['request']) {
      case 'report_dsr':
        return self::report_dsr($parameter);
        break;
    }
  }
  ...
  private function report_dsr($parameter)
  {
    $actual = self::$query->select('closing_set', array(
      'meta_data'
    ))
      ->where(array(
        '(created_at' => '>= ?',
      ...
      ->execute();
    foreach ($actual['response_data'] as $key => $value) {
      $actual['response_data'][$key]['meta_data'] = json_decode($value['meta_data'], true);
    }

    $month = self::$query->select('closing_set', array(
      'meta_data'
    ))
      ->where(array(
        '(created_at' => '>= ?',
      ...
      ->execute();
    foreach ($month['response_data'] as $key => $value) {
      $month['response_data'][$key]['meta_data'] = json_decode($value['meta_data'], true);
    }

    $year = self::$query->select('closing_set', array(
      'meta_data'
    ))
      ->where(array(
        '(created_at' => '>= ?',
        ...
        date('Y-m-d', strtotime(date('Y-01-01') . ' -1 day')),
        date('Y-m-d', strtotime(date('Y-m-d') . ' +1 day'))
      ))
      ->execute();
    foreach ($year['response_data'] as $key => $value) {
      $year['response_data'][$key]['meta_data'] = json_decode($value['meta_data'], true);
    }

    return array(
      'actual' => $actual['response_data'],
      'month' => $month['response_data'],
      'year' => $year['response_data']
    );
  }
  ...
  private function calculate($parameter)
  {
    ...
    $count_pax = self::$query->select('reservasi', array(
      ...
      ), array($parameter['from'], $parameter['to']))
      ->execute();
